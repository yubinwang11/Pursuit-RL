...
class Scenario(BaseScenario):
    ...
    def is_collision_boundary(self, agent):
        ...
        if agent.state.p_pos[0] > 1:
            collide = 1
            agent.state.p_pos[0] = 1 - infsmall
        if agent.state.p_pos[0] < -1:
            collide = 2
            agent.state.p_pos[0] = -1 + infsmall
        if agent.state.p_pos[1] > 1:
            collide = 3
            agent.state.p_pos[1] = 1 - infsmall
        if agent.state.p_pos[1] < -1:
            collide = 4
            agent.state.p_pos[1] = -1 + infsmall
        return collide
    ...
    def adversary_reward(self, agent, world):
        ...
        if shape:  # reward can optionally be shaped (decreased reward for increased distance from agents)
            for adv in adversaries:
                # CHANGE THIS! 0.1 min
                rew -= 1 * max([np.sqrt(np.sum(np.square(a.state.p_pos - adv.state.p_pos))) for a in agents])
        if agent.collide:
            for ag in agents:
                for adv in adversaries:
